import { useEffect, useState } from 'react';
import './index.css';

const blocks = [
  ['Q', 'W', 'E'],
  ['A', 'S', 'D'],
  ['Z', 'X', 'C'],
];

const audioUrls = {
  'Q': 'https://cdn.freecodecamp.org/testable-projects-fcc/audio/Heater-1.mp3',
  'W': 'https://cdn.freecodecamp.org/testable-projects-fcc/audio/Heater-2.mp3',
  'E': 'https://cdn.freecodecamp.org/testable-projects-fcc/audio/Heater-3.mp3',
  'A': 'https://cdn.freecodecamp.org/testable-projects-fcc/audio/Heater-4_1.mp3',
  'S': 'https://cdn.freecodecamp.org/testable-projects-fcc/audio/Heater-6.mp3',
  'D': 'https://cdn.freecodecamp.org/testable-projects-fcc/audio/Dsc_Oh.mp3',
  'Z': 'https://cdn.freecodecamp.org/testable-projects-fcc/audio/Kick_n_Hat.mp3',
  'X': 'https://cdn.freecodecamp.org/testable-projects-fcc/audio/RP4_KICK_1.mp3',
  'C': 'https://cdn.freecodecamp.org/testable-projects-fcc/audio/Cev_H2.mp3',
};

const inputToName = {
  'Q':'Heater 1',
  'W':'Heater 2',
  'E':'Heater 3',
  'A':'Heater 4',
  'S':'Clap',
  'D':'Open HH',
  'Z':'Kick n\'hat',
  'X':'Kick',
  'C':'Closed HH'
}

function App() {
  const [activeButton, setActiveButton] = useState(null);

  const playSound = (key) => {
    if(inputToName[key]){
    document.getElementById('display').innerText = inputToName[key];
    }
    const audioElement = document.getElementById(key);
    if (audioElement) {
      audioElement.currentTime = 0;
      audioElement.play(); // Play the audio element
      setActiveButton(key);
      setTimeout(() => {
        setActiveButton(null);
      }, 100);
    }
  };

  useEffect(() => {
    const handleKeyDown = (event) => {
      const key = event.key.toUpperCase();
      playSound(key);
    };

    const handleKeyUp = () => {
      setActiveButton(null);
    };

    window.addEventListener('keydown', handleKeyDown);
    window.addEventListener('keyup', handleKeyUp);

    return () => {
      window.removeEventListener('keydown', handleKeyDown);
      window.removeEventListener('keyup', handleKeyUp);
    };
  }, []);

  return (
    <>
      <div id="drum-machine">
        <div id='title-container'>
          <h1>DRUM <br />MACHINE</h1>
          <span id='display'>music!!!</span>
        </div>
        {blocks.map((row, rowIndex) => (
          <div id="row" key={rowIndex}>
            {row.map((value, numberIndex) => (
              <button
                id="hi"
                key={numberIndex}
                className={activeButton === value ? 'active drum-pad' : 'drum-pad'}
                onClick={() => playSound(value)} // Play sound on button click
              >
                {value}
                <audio
                  className="clip"
                  id={value} // Make sure the ID matches the key
                  src={audioUrls[value]} // Set audio source
                ></audio>
              </button>
            ))}
          </div>
        ))}
      </div>
    </>
  );
}

export default App;